...
class MenuLeftSideFirst(PushButton):
    ...
    def __init__(
        ...
        self._icon_press = self.icon_to_press(self._icon.toImage()) 
        self._move_letf = 14
        self._move_space = 10
    ...
    def icon_to_press(self, icon: QImage) -> QPixmap:
        
        for x in range(icon.width()):
            for y in range(icon.height()):
                dot = icon.pixelColor(x, y)
                r = 255 - dot.red()
                g = 255 - dot.green()
                b = 255 - dot.blue()
                a = dot.alpha()
                icon.setPixelColor(x, y, QColor(r, g, b, a))
        return QPixmap(icon)
...
class MenuLeftSideSecondary(PushButton):
    ...
    def paintEvent(self, event: QPaintEvent) -> None: 
        ...
        painter.setRenderHints(QPainter.Antialiasing | QPainter.TextAntialiasing)
        if self.isChecked():
            self.draw(event, painter, "#ffffff")
        else:
            self.draw(event, painter, "#000000")
    
    def draw(self, event: QPaintEvent, painter: QPainter, font_color: str) -> None:
        # 画文字
        painter.setFont(self._font)
        painter.setPen(QColor(font_color))
        painter.drawText(
...
class MenuLeftSideSecondaryList:
    ...
    def set_setHidden(self, item: QListWidgetItem) -> None:
        if MenuLeftSideSecondaryList._item_hidden_num is None:
            item.setHidden(not item.isHidden())
            MenuLeftSideSecondaryList._item_hidden_num = MenuLeftSideSecondaryList._item_hidden.index(item)
        else:
            if MenuLeftSideSecondaryList._item_hidden_num != MenuLeftSideSecondaryList._item_hidden.index(item):
                item_old = MenuLeftSideSecondaryList._item_hidden[MenuLeftSideSecondaryList._item_hidden_num]
